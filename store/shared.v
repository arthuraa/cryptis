...
Definition db_server_ready kI kR db : iProp :=
  ∃ os, ⌜db = DB.to_db os⌝ ∗ rep_copy kI kR N [] os.
...
iMod (rep_main_alloc (N := N) kI (kR := kR) [] with "kI_token")
...
Definition store_call_pred kI kR si ts : iProp :=
  ∃ t1 t2 os,
    ⌜ts = [t1; t2]⌝ ∗
    rep_update kI kR N [] os (os ++ [Store t1 t2]).
...
Definition create_call_pred kI kR si ts : iProp :=
  ∃ t1 t2 os, ⌜ts = [t1; t2]⌝ ∗ ⌜DB.to_db os !! t1 = None⌝ ∗
    rep_update kI kR N [] os (os ++ [Create t1 t2]).
