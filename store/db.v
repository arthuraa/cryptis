...
Definition hist_auth γ N os : iProp Σ :=
  nown γ (N.@"hist")
         (● to_max_prefix_list os ⋅ ◯ to_max_prefix_list os).

Definition hist_frag γ N os : iProp Σ :=
  nown γ (N.@"hist")
         (◯ to_max_prefix_list os).

Local Instance persistent_hist_frag γ N os : Persistent (hist_frag γ N os).
Proof. apply _. Qed.

Lemma hist_auth_frag γ N os :
  hist_auth γ N os -∗
  hist_frag γ N os.
Proof. by iIntros "[_ Hown]". Qed.

Lemma hist_frag_agree γ N os1 os2 :
...
Lemma hist_frag_prefix_of os1 os2 γ N :
  os1 `prefix_of` os2 →
  hist_frag γ N os2 -∗
  hist_frag γ N os1.
Proof.
...
Definition state_auth γ N db : iProp Σ :=
  nown γ (N.@"state") (db_state db).

Definition mapsto γ N t1 t2 : iProp Σ :=
  nown γ (N.@"state") (db_singleton t1 t2).

Definition free_at γ N T : iProp Σ :=
  nown γ (N.@"state") (db_free T).

Lemma free_at_diff γ N T1 T2 :
  T1 ⊆ T2 →
  free_at γ N T2 ⊣⊢ free_at γ N T1 ∗ free_at γ N (T2 ∖ T1).
Proof.
move=> sub. by rewrite /free_at db_free_diff // nown_op.
Qed.

Lemma state_auth_mapsto γ N db t1 t2 :
  state_auth γ N db -∗
  mapsto γ N t1 t2 -∗
  ⌜db !! t1 = Some t2⌝.
Proof.
...
Lemma state_auth_update t2' γ N db t1 t2 :
  state_auth γ N db -∗
  mapsto γ N t1 t2 ==∗
  state_auth γ N (<[t1 := t2']>db) ∗
  mapsto γ N t1 t2'.
Proof.
iIntros "Hauth Hfrag".
...
Lemma state_auth_create t1 t2 γ N db :
  state_auth γ N db -∗
  free_at γ N {[t1]} ==∗
  state_auth γ N (op_app db (Create t1 t2)) ∗
  mapsto γ N t1 t2.
Proof.
...
Definition client_view γ N n : iProp Σ :=
  ∃ os,
    ⌜n = length os⌝ ∗
    hist_auth γ N os ∗
    state_auth γ N (to_db os).

Definition update_at γ N n t1 t2 : iProp Σ :=
  ∃ os, ⌜n = length os⌝ ∗
        hist_frag γ N (os ++ [Update t1 t2]).

Definition create_at γ N n t1 t2 : iProp Σ :=
  ∃ os, ⌜n = length os⌝ ∗
        hist_frag γ N (os ++ [Create t1 t2]).

Definition server_view γ N n db : iProp Σ :=
  ∃ os, ⌜n = length os⌝ ∗
        ⌜db = to_db os⌝ ∗
        hist_frag γ N os.
...
Lemma update_client t2' γ N n t1 t2 :
  client_view γ N n -∗
  mapsto γ N t1 t2 ==∗
  client_view γ N (S n) ∗
  update_at γ N n t1 t2' ∗
  mapsto γ N t1 t2'.
Proof.
...
Lemma update_server γ N n db t1 t2 :
  server_view γ N n db -∗
  update_at γ N n t1 t2 -∗
  server_view γ N (S n) (<[t1 := t2]>db).
Proof.
...
Lemma create_client t1 t2 γ N n :
  client_view γ N n -∗
  free_at γ N {[t1]} ==∗
  create_at γ N n t1 t2 ∗
  client_view γ N (S n) ∗
  mapsto γ N t1 t2.
Proof.
...
Lemma create_server γ N n db t1 t2 :
  db !! t1 = None →
  server_view γ N n db -∗
  create_at γ N n t1 t2 -∗
  server_view γ N (S n) (<[t1 := t2]>db).
...
iAssert (hist_frag γ N os') as "#frag''".
{ iApply (hist_frag_prefix_of with "frag'").
...
Definition stored_at γ N n t1 t2 : iProp Σ :=
  ∃ os, ⌜n = length os⌝ ∗
        hist_frag γ N os ∗
        ⌜to_db os !! t1 = Some t2⌝.

Lemma load_client γ N n t1 t2 t2' :
  client_view γ N n -∗
  mapsto γ N t1 t2 -∗
  stored_at γ N n t1 t2' -∗
  ⌜t2' = t2⌝.
Proof.
...
Lemma load_server γ N n db t1 t2 :
  db !! t1 = Some t2 →
  server_view γ N n db -∗
  stored_at γ N n t1 t2.
Proof.
...
Lemma client_view_server_view γ N n :
  client_view γ N n -∗ ∃ db,
  server_view γ N n db.
Proof.
...
Lemma server_view_to_0 γ N n db :
  server_view γ N n db -∗
  server_view γ N 0 ∅.
Proof.
...
Lemma server_view_0 γ N db :
  server_view γ N 0 db -∗
  ⌜db = ∅⌝.
Proof.
