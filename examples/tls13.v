...
- wp_bind (mk_nonce _); iApply (wp_mk_nonce (λ _, True)%I (λ _, True)%I) => //.
  iIntros (cn) "_ #p_cn _ _ _ token"; wp_list; wp_term_of_list.
  wp_tag.
...
- subst psk.
  wp_bind (mk_nonce _); iApply (wp_mk_nonce (λ _, True)%I (λ _, True)%I) => //.
  iIntros (a) "_ #pred_a _ _ _ token"; wp_list; wp_term_of_list.
  wp_tag; iModIntro.
...
- subst g'.
  ...
  wp_bind (mk_nonce _); iApply (wp_mk_nonce (λ _, True)%I (λ _, True)%I) => //.
  iIntros (sn) "_ #p_sn _ _ _ token"; wp_list; wp_term_of_list.
  wp_tag; iModIntro.
...
iDestruct "token" as "[token _]".
iMod (SParams.public_hello with "s_ctx wf_sp []")
  as "(#p_hello & #sess)" => //.
