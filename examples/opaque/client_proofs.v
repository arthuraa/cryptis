...
wp_apply wp_send => //.
  ...
  - iApply public_TExp_iff.
      by case.
      by exact: (proj1 (is_trueP _) (nonce_Nmul Hnoncer)).
    do !iSplit => //.
  ...
  - iApply public_TExp_iff.
      by case.
      by exact: (proj1 (is_trueP _) (nonce_Nmul Hnoncex_u)).
    do !iSplit => //.
    + by iApply minted_TInt.
    + iApply exp_pred_intro1.
      iApply "Hexpx_u"; iPureIntro.
      have Nm : is_true (negb (is_mul x_u)) := nonce_Nmul Hnoncex_u.
      rewrite (_ : TExp g x_u = TExpN g [x_u]); last by rewrite /TExpN TMulN1.
...
wp_apply wp_hl_inv_term; first by exact: (nonce_Nmul Hnoncer).
...
iModIntro; iSplit.
  ...
  - iDestruct "contra" as "(_ & contra & _)".
    ...
    by iApply (public_opaque_secret _ (nonce_Nmul Hnoncep_u) (nonce_Nmul Hnoncep_s) p_u_s p_u_sV) => //.
  - do !iSplit => //.
...
- iPureIntro => /Hfreshr; apply.
  ...
  split.
    by right; left.
  apply: subterm_TExp_exp; [done | exact: (nonce_Nmul Hnoncer) | exact: STRefl].
- rewrite minted_of_list /=
