...
Context `{!cryptisGS Σ, !heapGS Σ, !spawnG Σ, !tlockG Σ, !nsl_dhGS Σ}.
...
Definition nsl_dh_game_inv rl x : iProp := ∃ si,
  ⌜(si_key si : term) = x⌝ ∧
  term_meta (si_share_of rl si) (gameN.@"fresh") ().

Lemma nsl_dh_game_fresh rl si E :
  ↑gameN.@"fresh" ⊆ E →
  term_token (si_share_of rl si) E -∗
  fresh_term (nsl_dh_game_inv rl) (si_key si) ∗
...
Lemma wp_do_init_loop c lcomp vset skI skR :
  ...
  cryptis_ctx -∗
  nsl_dh_ctx -∗
  nsl_dh_pred gameN (λ _ _ _ _, True)%I -∗
  minted skI -∗
  minted skR -∗
  inv gameN (game_inv lcomp skI skR) -∗
  is_lock_term_set (nsl_dh_game_inv Init) vset -∗
  {{{ True }}}
...
iPoseProof (nsl_dh_game_fresh Init with "token")
...
wp_lam. wp_pures.
wp_apply (wp_new_lock_term_set (nsl_dh_game_inv Init)) => //.
...
Lemma wp_do_resp_loop c lcomp set skI skR :
  ...
  minted skR -∗
  inv gameN (game_inv lcomp skI skR) -∗
  is_lock_term_set (nsl_dh_game_inv Resp) set -∗
  {{{ True }}}
...
wp_pures.
iPoseProof (nsl_dh_game_fresh Resp with "token")
  as "[fresh token]"; first solve_ndisj.
...
iIntros "#? #? #? #? #? #? #? %Φ _ !> post".
wp_lam. wp_pures.
wp_apply (wp_new_lock_term_set (nsl_dh_game_inv Resp)) => //.
iIntros "%set #set". wp_pures.
...
{ iFrame. }
wp_pures. wp_apply wp_send => //; first by iApply public_aenc_key.
wp_pures. wp_apply wp_send => //; first by iApply public_aenc_key.
wp_pures.
...
Definition F : gFunctors :=
  #[heapΣ; spawnΣ; cryptisΣ; tlockΣ; nsl_dhΣ].

Lemma nsl_dh_secure σ₁ σ₂ (v : val) t₂ e₂ :
...
iIntros (? ? c) "#ctx #chan (aenc & _ & _ & _)".
iMod (nsl_dhGS_alloc with "aenc") as (?) "(#? & nsl_tok & _)"; eauto.
iMod (nsl_dh_pred_set gameN (λ _ _ _ _, True)%I with "nsl_tok")
  as "[#? ?]"; eauto.
