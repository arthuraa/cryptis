...
Definition initiator : val := λ: "c" "skI" "pkR" "N" "data",
  let: "pkI"  := pkey "skI" in
  let: "a"    := mk_nonce #() in
  ...
  let: "m3_body" := term_of_list ["ga"; "gb"; "pkR"; "N"; "data"] in
...
Definition responder_accept : val := λ: "c" "skR" "ga" "pkI" "N",
  ...
  list_match: ["ga'"; "gb'"; "pkR'"; "N'"; "data"] := "m3" in
  guard: eq_term "ga" "ga'" && eq_term "gb" "gb'" && eq_term "pkR" "pkR'"
         && eq_term "N" "N'" in
  let: "gab" := texp "ga" "b" in
  let: "secret" := term_of_list ["pkI"; "pkR"; "ga"; "gb"; "gab"] in
  SOME (derive_senc_key "secret", "data").

Definition responder : val := λ: "c" "skR" "N",
  let: "res" := responder_wait "c" in
  let: "ga"  := Fst "res" in
  let: "pkI" := Snd "res" in
  bind: "res" := responder_accept "c" "skR" "ga" "pkI" "N" in
  let: "kS" := Fst "res" in
  let: "data" := Snd "res" in
  SOME ("pkI", "kS", "data").
