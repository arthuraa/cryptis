...
Definition session l (kI kR nI nR : term) : iProp :=
  l ↦□ (Spec.of_list [TKey Enc kI; TKey Enc kR; nI; nR] : val).

Global Instance session_persistent l kI kR nI nR :
  Persistent (session l kI kR nI nR).
Proof. apply _. Qed.
...
Definition msg2_pred lR kI m2 : iProp :=
  ∃ nI nR kR,
    ...
    session lR kI kR nI nR ∧
    (public nR ↔ ▷ corrupt kI kR).

Definition msg3_pred lI lR kR nR : iProp :=
  ∃ nI kI,
    session lI kI kR nI nR ∧
    (honest kR -∗ ▷ session lR kI kR nI nR).
...
Lemma public_msg3E lI lR kI kR nI nR :
  ...
  □ (public nR ↔ ▷ corrupt kI kR) -∗
  session lR kI kR nI nR -∗
  □ (honest kI -∗ ▷^2 session lI kI kR nI nR).
Proof.
...
wp_pures. iApply ("Hpost" $! (Some (nI, nR))).
iModIntro. by eauto.
Qed.
...
Lemma wp_resp c lI lR v0 kR :
  ...
  {{{ ts, RET (repr ts);
      if ts is Some (pkI, nI, nR) then ∃ kI,
        ⌜pkI = TKey Enc kI⌝ ∗
        public (TKey Enc kI) ∗
        session lR kI kR nI nR ∗
        □ (honest kI -∗ ▷^2 session lI kI kR nI nR)
...
wp_pures. wp_list. iApply ("Hpost" $! (Some (TKey Enc kI, nI, nR))).
...
Definition game : val := λ: "mkchan",
  ...
  let: "res" := init "c" "lI" "skI" "pkI" "pkR'" ||| resp "c" "lR" "skR" "pkR" in
  bind: "resI" := Fst "res" in
  bind: "resR" := Snd "res" in
  let: "pkI'" := Fst (Fst "resR") in
...
iApply (wp_par (λ v, ∃ a : option (term * term), ⌜v = repr a⌝ ∗ _)%I
               (λ v, ∃ a : option (term * term * term), ⌜v = repr a⌝ ∗ _)%I
          with "[HlI] [HlR]").
...
iModIntro.
wp_pure credit:"c1".
case: a => [[nI1 nR1]|]; wp_pure credit:"c2"; wp_pures; last by eauto.
case: b => [[[pkI' nI2] nR2]|]; wp_pures; last by eauto.
...
iAssert (|={⊤}=> ⌜pkR = TKey Enc kR' ∧ pkI = TKey Enc kI' ∧ nI1 = nI2 ∧ nR1 = nR2⌝)%I
  with "[c1 c2]" as ">%e".
{ case: succ => [[<-]|[<-]].
  - iSpecialize ("sessR" with "s_skR").
    iMod (lc_fupd_elim_later with "c1 sessR") as "{sessR} sessR".
    iPoseProof (mapsto_agree with "sessR sessR'") as "%e".
    case/val_of_term_inj/Spec.of_list_inj: e => <- <- <-.
    by eauto.
  - iSpecialize ("sessI'" with "s_skI").
